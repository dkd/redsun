    ...
    module JournalPatch
      def self.included(base) # :nodoc:
        ...
        base.class_eval do
          searchable do
            ...
            # Notes
            text :notes, stored: true, boost: 4 do
              result = ""
              result += notes.gsub(/[[:cntrl:]]/, ' ').scan(/[[:print:][:space:]]/).join if notes.present?
            end
      ...
      module InstanceMethods
        def indice_for_sunspot
          journalized.journals.map(&:id).index(id) + 1 if journalized.try(:journals) && journalized.journals.any?
        end
...
unless Journal.included_modules.include?(RedmineRedsun::Patches::JournalPatch)
  Journal.include RedmineRedsun::Patches::JournalPatch
end
